...
12. 当前运行时也会检测项目级 `.amp/settings.json` 与 `.amp/plugins/`，并提示这些 workspace 层可能 augment / override 全局 Claude Island plugin。
13. 当前 `amp-stream` 会在解析型第二入口里强制 `TERM=dumb`、`NO_COLOR=1`、`CLICOLOR=0` 并附带 `--no-color`，尽量压低终端控制序列噪音。
14. 当前 `amp-stream` 也会在“没有任何可解析 JSON 行”时，把首条非 JSON 输出收敛成明确错误信息，而不是只剩终端控制序列。
15. 基于本机真实样本，当前 helper 也会把 `Cannot reach Amp servers` 这类网络前置错误收敛成更明确的诊断，而不是只显示通用失败。
16. 下一步仍建议补更深的 `--stream-json` watcher/事件细化，用于无 plugin 模式或更稳定的会话分组。
